...
class CanonicalValidator {
  ...
  static _validateSchemaContract(ctx, bag) {
    ...
    // E116 — sroi_metrics must have {status, calculated} shape
    if (p.sroi_metrics && this._isPlainObject(p.sroi_metrics)) {
      if (!('status' in p.sroi_metrics)) {
        this._push(bag, 'error', {
          code: 'E116', layer: 2, category: 'schema_contract',
          field: 'sroi_metrics.status', path_type: 'exact',
          message: "sroi_metrics harus punya field 'status'.",
          why_it_matters: "Financial engine membaca sroi_metrics.status — tanpa field ini pipeline crash.",
          action: 'Tambahkan: "status": "pending" di dalam sroi_metrics.',
        });
      }
      if (!('calculated' in p.sroi_metrics)) {
        this._push(bag, 'error', {
          code: 'E116', layer: 2, category: 'schema_contract',
          field: 'sroi_metrics.calculated', path_type: 'exact',
          message: "sroi_metrics harus punya field 'calculated'.",
          why_it_matters: "Financial engine mengisi sroi_metrics.calculated — tanpa struktur ini pipeline crash.",
          action: "Tambahkan field 'calculated' dengan nilai awal 0 di dalam sroi_metrics.",
        });
      }
      // Deteksi format flat lama
      const flatKeys = ['blended_sroi','sroi_2023','sroi_2024','sroi_2025'];
      if (flatKeys.some(k => k in p.sroi_metrics)) {
        this._push(bag, 'error', {
          code: 'E116', layer: 2, category: 'schema_contract',
          field: 'sroi_metrics', path_type: 'exact',
          message: "sroi_metrics menggunakan format lama (flat) yang tidak didukung.",
          why_it_matters: "Financial engine mengharapkan format {status, calculated} — format flat akan crash.",
          action: 'Ganti dengan: { "status": "pending", "calculated": { "sroi_blended": 0, "per_year": [] ... } }',
        });
      }
    }

    // E117 — program_identity required fields for full_assembler
    const PI_ASSEMBLER_FIELDS = ['company', 'program_tagline', 'period_label', 'target_group'];
    if (this._isPlainObject(p.program_identity)) {
      PI_ASSEMBLER_FIELDS.forEach(f => {
        if (!(f in p.program_identity) || p.program_identity[f] === '' || p.program_identity[f] === null) {
          this._push(bag, 'error', {
            code: 'E117', layer: 2, category: 'schema_contract',
            field: `program_identity.${f}`, path_type: 'exact',
            message: `Field '${f}' wajib ada di program_identity.`,
            why_it_matters: `full_assembler.js membaca program_identity.${f} langsung — tanpa field ini assembler crash.`,
            action: `Tambahkan '${f}' ke program_identity.`,
            example_fix: {
              company: 'PT Pertamina Lubricants',
              program_tagline: 'Tagline program',
              period_label: '2023–2025',
              target_group: 'Kelompok sasaran utama',
            }[f],
          });
        }
      });
  ...
  static _validateDataQuality(ctx, bag) {
    ...
    // W011 — activities dict format (not recommended)
    const _actsRaw = p.activities;
    if (_actsRaw && this._isPlainObject(_actsRaw) && !Array.isArray(_actsRaw)) {
      this._push(bag, 'warning', {
        code: 'W011', layer: 3, category: 'data_quality',
        field: 'activities', path_type: 'exact',
        message: "activities menggunakan format dict per tahun — format array lebih aman.",
        why_it_matters: "Format dict bisa menyebabkan inkonsistensi di beberapa step pipeline.",
        action: "Ubah activities menjadi array of objects: [{ year: 2023, name: '...', activity_scope: [...] }, ...]",
      });
    }
